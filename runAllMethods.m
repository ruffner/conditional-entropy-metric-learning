...
for i  = 1:num_runs
    ...
    disp(sprintf('CEML kNN cross-validated accuracy = %f', acc(CEML)));
    ...
    disp(sprintf('ITML kNN cross-validated accuracy = %f', acc(ITML)));
    ...
    disp(sprintf('NCA kNN cross-validated accuracy = %f', acc(NCA)));
    ...
    disp(sprintf('MCML kNN cross-validated accuracy = %f', acc(MCML)));
    ...
    disp(sprintf('LMNN kNN cross-validated accuracy = %f', acc(LMNN)));
    ...
    disp(sprintf('InvCov kNN cross-validated accuracy = %f', acc(invCov)));
    
    
    %%% Eucledian Distance
    acc(i,Euclidean) = CrossValidateKNN2(labels, X, @(labels, X) euclideanWrap(X, labels), knn_neighbor_size, yTest, xTest);
    disp(sprintf('Euclidean kNN cross-validated accuracy = %f', acc(Euclidean)));
